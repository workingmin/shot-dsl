...
- 双人巷战、拳击脸部特写、三人混战、跟拍追逐、环绕建立镜头和 6 分 30 秒广播体操六个快捷示例。
...
默认 `human-mannequin` 来自 Mesh2Motion/Quaternius 的 CC0 humanoid 资产，内置 16 个经过裁剪/重定向的动作 Clip，覆盖移动、搏击和基础体操。运行时将模型统一归一为 1.78 米、脚底对齐地面，并为每个 actor 克隆独立骨架。`robot-expressive` 继续保留为开发备用资产，但不再用于快捷演示。
