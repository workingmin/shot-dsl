...
export const loadExamples = async () => {
  const response = await readResponse(await fetch('/examples/storyboard/manifest.json', { cache: 'no-store' }), '场景示例清单')
...
export const loadExampleSource = async example => {
  const path = `/examples/storyboard/${encodeURIComponent(example.file)}`
